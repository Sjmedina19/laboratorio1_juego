...
class MenuScene:
    def __init__(self, game, screen_width, screen_height, message=""):
        self.game = game
    ...
    def load_menu_background(self):
        path = os.path.join("assets","imagenes","menu.png")
    ...
    def execute_selected(self):
        ...
        if option=="Nueva partida": return {"action":"new_game"}
        elif option=="Continuar": return {"action":"continue_game"}
        elif option=="Opciones": return {"action":"settings"}
        elif option=="Salir": return {"action":"quit"}
        return None
    ...
    def draw(self, screen):
        ...
        for i,option in enumerate(self.options):
            ...
            if i==self.selected: pygame.draw.rect(screen,(210,180,255),rect,2,border_radius=8)
            text = self.font_option.render(option,True,text_color)
            screen.blit(text,text.get_rect(center=rect.center))
        if self.message:
            msg = self.font_message.render(self.message,True,(255,235,130))
            screen.blit(msg,msg.get_rect(center=(self.screen_width//2,self.screen_height-50)))
